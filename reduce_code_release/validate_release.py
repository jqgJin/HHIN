"""Validate dataset archives and, optionally, an audited result bundle."""
...
def validate_results(results_dir: Path, failures: list[str]) -> dict:
    ...
    selected_root = results_dir / "semantic_selection"
    selected_path = first_existing(
        selected_root / "selected_thresholds_for_clustering.csv",
        selected_root / "selected_thresholds.csv",
    )
    ...
    clustering_root = results_dir / "clustering"
    clustering_path = first_existing(
        clustering_root / "clustering_selected_summary.csv",
        clustering_root / "clustering_summary.csv",
    )
    ...
    neighbor_root = results_dir / "label_neighborhood"
    neighbor_path = first_existing(
        neighbor_root / "prediction_selected_summary.csv",
        neighbor_root / "label_neighborhood_summary.csv",
    )
...
def main() -> None:
    ...
    parser.add_argument("--skip-results", action="store_true", help="Validate dataset archives without requiring an audited result bundle")
    ...
    report = {
        ...
        "results": {} if args.skip_results else validate_results(args.results_dir.resolve(), failures),
    }
