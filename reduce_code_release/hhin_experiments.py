...
def run_kmeans(X, n_clusters, seed, n_init=10):
    return KMeans(n_clusters=n_clusters, random_state=seed, n_init=n_init, max_iter=300).fit_predict(X)
...
def _retrieval_row_metrics(sim_o, sim_r, query_idx: int, k_values):
    ...
    for k in k_values:
        ...
        if kk <= 0:
            ...
            out[f'ndcg@{k}'] = 1.0
            continue
        top_o = np.argpartition(-so, kk - 1)[:kk]
        top_r = np.argpartition(-sr, kk - 1)[:kk]
        out[f'overlap@{k}'] = len(set(top_o.tolist()) & set(top_r.tolist())) / kk
        true_scores = np.where(np.isfinite(so), so, 0.0)
        pred_scores = np.where(np.isfinite(sr), sr, -1e9)
        out[f'ndcg@{k}'] = _ndcg_from_scores(pred_scores, true_scores, kk)
    return out
...
def run_kbisim_aligned_validation(args):
    """Run K-bisimulation-style downstream maintainability checks.
    ...
    print('[HHIN-KBISIM] Running clustering maintainability...')
    clustering_summary = run_clustering_selected(args)
    print('[HHIN-KBISIM] Running label-prediction maintainability...')
...
def build_arg_parser():
    ...
    ap.add_argument('--selection-rule', choices=['normalized_risk', 'hard'], default='normalized_risk',
                    help='normalized_risk selects max reduction under normalized semantic risk <= 1; hard uses legacy boolean constraints.')
