...
def layered_similarity_matrices(X, T, term_sim='cosine'):
    ...
    sim_attr = cosine_matrix_sparse(X)
...
def _passes_layered_guard(cand: int, cluster: List[int], sim_attr, sim_term, tau_attr: float, tau_term: float) -> bool:
    ...
    if sim_attr is not None:
        if float(np.min(sim_attr[cand, cluster])) < tau_attr - 1e-8:
            return False
...
def ndcg_at_k(pred_scores, true_scores, k=10):
    ...
    gains = true_scores[order_pred]
    discounts = 1.0 / np.log2(np.arange(2, k + 2))
    dcg = float(np.sum(gains * discounts))
    order_true = np.argsort(-true_scores)[:k]
    ideal = float(np.sum(true_scores[order_true] * discounts))
    return dcg / ideal if ideal > 1e-12 else 1.0
...
def ranking_metrics(sim_o, sim_r, self_idx, k_values=(1, 5, 10)):
    ...
    for k in k_values:
        ...
        if kk <= 0:
            out[f'top{k}'] = 1.0
            continue
        top_o = np.argpartition(-so, kk)[:kk]
        top_r = np.argpartition(-sr, kk)[:kk]
...
def main():
    ap = argparse.ArgumentParser(description='HHIN semantic maintainability benchmark with layered Guard, P95E/P99E constraints, and optional CPU parallelism')
    ...
    results_df.to_csv(os.path.join(args.out_dir, 'semantic_maintainability_summary.csv'), index=False)
    perpath_df.to_csv(os.path.join(args.out_dir, 'semantic_maintainability_per_path.csv'), index=False)
    pertype_df.to_csv(os.path.join(args.out_dir, 'semantic_maintainability_per_type.csv'), index=False)
    part_df.to_csv(os.path.join(args.out_dir, 'semantic_maintainability_struct_partition_summary.csv'), index=False)
    joint_rec_df.to_csv(os.path.join(args.out_dir, 'semantic_maintainability_recommended_thresholds.csv'), index=False)
    ...
    if args.write_pareto:
        pareto_df.to_csv(os.path.join(args.out_dir, 'semantic_maintainability_pareto_frontier.csv'), index=False)
    ...
    with open(os.path.join(args.out_dir, 'run_summary.txt'), 'w', encoding='utf-8') as f:
        f.write('HHIN reduction-oriented semantic maintainability benchmark with layered Guard, P95E/P99E, and CPU parallelism\n')
        ...
        f.write('Primary downstream task: similarity retrieval maintainability based on PathSim/HeteSim\n')
